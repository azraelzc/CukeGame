...
local function sortingOrderUI(panel,uiType,isShow)
    if GOUtil.GetComponent(panel.prefab,"Canvas") == nil then
        local rootCanvas = GOUtil.AddComponent(panel.prefab,"Canvas")
        GOUtil.AddComponent(panel.prefab,"GraphicRaycaster")
        rootCanvas.overrideSorting = true
    end
    local baseOrder = UIManager.GetLayerSortinOrder(uiType)
    ...
    for i=0,canvases.Length - 1 do
        ...
        if canvas.sortingOrder < baseOrder then
            canvas.sortingOrder = canvas.sortingOrder + baseOrder
            if addOrder > 0 then
                canvas.sortingOrder = canvas.sortingOrder + addOrder
            end
        else
            canvas.sortingOrder = canvas.sortingOrder + addOrder
        end
    ...
    --particle
    local renderers = GOUtil.GetComponentsInChildren(panel.prefab,"Renderer")
    for i=0,renderers.Length - 1 do
        local renderer = renderers[i]
        if renderer.sortingOrder < baseOrder then
            renderer.sortingOrder = renderer.sortingOrder + baseOrder
            if addOrder > 0 then
                renderer.sortingOrder = renderer.sortingOrder + addOrder
            end
        else
            renderer.sortingOrder = renderer.sortingOrder + addOrder
        end
